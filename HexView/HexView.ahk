...
;
;	About:
;			- v3.1 by majkinetor. See http://www.autohotkey.com/forum/topic17858.html
...
HexView_Create(pHexa, pAsci, pActiveTab) {
  ...
  ;--------- MAIN SCREEN ---------------
	...
	Gui, %HexView_Gui%:Add, Text,	xs+5	y+5	  w%iX%				vHexView_status,
	HexView_Subclass(HexView_eHexa, "HexView_wndProc", "F")

	iX+=3	;tab control starts after status
	Gui, %HexView_GUI%:Add, Tab,	x%iX%	y-30  w%asciW% h%tmp%   0x100 +0x8 vHexView_tab g_HexView_TabChange -Wrap, ascii|struct|skin|opt|?


	;slider and dummy edit
	Gui, %HexView_GUI%:Tab,
	Gui, %HexView_Gui%:Add, Slider, x+3 ys h%iY% w20    vHexView_slider g_HexView_OnSlider  AltSubmit NoTicks Page3 Vertical
	Gui, %HexView_Gui%:Add, Edit,   x-100  h%iX% w0		HWNDHexView_eDummy -TabStop, %dummy%

	;add tab buttons
	iY+=5
	Gui, %HexView_GUI%:Font, bold 
	Gui, %HexView_Gui%:Add, Text,	0x8000 x%hexW% y%iY% h20 vHexView_tab1  g_HexView_OnTabButtonClick section, ascii
	Gui, %HexView_Gui%:Add, Text,	0x8000 x+10 ys h20	 vHexView_tab2  g_HexView_OnTabButtonClick, struct
	Gui, %HexView_Gui%:Add, Text,	0x8000 x+45 ys h20	 vHexView_tab3  g_HexView_OnTabButtonClick, skin
	Gui, %HexView_Gui%:Add, Text,	0x8000 x+10 ys h20	 vHexView_tab4  g_HexView_OnTabButtonClick, opt
	Gui, %HexView_Gui%:Add, Text,	0x8000 x+10 ys h20	 vHexView_tab5  g_HexView_OnTabButtonClick, ?
 ...
 ;TAB STRUCTS
	...
	Gui, %HexView_Gui%:Add, ComboBox,	xs ys	w%asciw%		g_HexView_OnStructChange vHexView_cbStructs,	 ;%HexView_lastStruct%||
	Gui, %HexView_Gui%:Add, ListView,	xs y+0	w%asciw% h%iY%	vHexView_lvMembers g_HexView_OnListView	 ReadOnly %hdr% 0x1 +LV0x100 -Multi,	Value|Name
...
HexView_OnTxtClick( ctrl ) {
	...
	{
		clr := %A_GuiControl%
		if !CmnDlg_ChooseColor(clr, HexView_hwnd)
			return
	...
	if ctrl = HexView_pFont
	{
		...
		if !CmnDlg_ChooseFont(x, y, clr, HexView_hwnd)
			return
...
HexView_Struct_GetDef( sName ) {
	...
	loop, parse, sInfo, |, `n
	{
		...
		HexView_aMember[%A_Index%]_name := SubStr(A_LoopField, 1, StrLen(A_LoopFiled)-2)
		type := SubStr(A_LoopField, -0, 2)

		if (type = "D")
			 HexView_aMember[%A_Index%]_byteNo := 4
		else if (type ="W") 
				HexView_aMember[%A_Index%]_byteNo := 2
			 else if (type="B")
					HexView_aMember[%A_Index%]_byteNo := 1

		HexView_aMember[0] := A_Index
	}
...
HexView_Struct_Translate() {
 ...
 ;calculate members
	...
	loop, %HexView_aMember[0]%
	{
		byteNo := HexView_aMember[%A_Index%]_byteNo
		...
		LV_Add("", value, HexView_aMember[%A_Index%]_name)
		HexView_tooltip .= 	HexView_aMember[%A_Index%]_name " = " value "`n"
	}
...
HexView_Size:
	Anchor("HexView_asci",		"hw")
	Anchor("HexView_hexa",		"h" )
	Anchor("HexView_status",	"y" )
	Anchor("HexView_tab",		"h" )
	Anchor("HexView_slider",	"xh")
	Anchor("HexView_lvMembers", "wh")
	Anchor("HexView_cbStructs", "w")

	Loop, 9
		Anchor("HexView_tab" A_Index, "y")
return
...
HexView_About() {
	msg := "By Miodrag Milic`n"
		 . "miodrag.milic@gmail.com`n`n"
		 . "code.r-moth.com`nwww.r-moth.com`nr-moth.deviantart.com`n`n"
		 . "2007`nBelgrade`n`n"
